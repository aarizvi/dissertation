...
\noindent The final table can be found on UB CCR at:    
\noindent `/projects/rpci/lsuchest/abbasriz/candidate_gene/ \`  
`result_files_cg/final_table/cg_snptable.txt`.  

\noindent The ultimate goal of this study was to test all of the candidate SNPs in DISCOVeRY-BMT to replicate or validate the original studies' findings. Replication means that the phenotype and population are the same. Validation means that the phenotype is the same but the study population is not necessarily the same (i.e. different ethnic group). And also, because these studies essentially had gene based hypotheses, we sought to test DISCOVeRY-BMT survival data using gene-based statistical approaches (VEGAS2 software) to test whether the aggregation of the entire gene locus was significant or not. 

\noindent Here we present our data analysis.

\singlespacing
```{bash Awk to retrieve final list after results, eval=F}
...
\singlespacing
```{r Pseudo-Loading Genes with Locations, eval=F}
# load gene list
...
\noindent We need to grab the typed and imputed SNPs from DISCOVeRY-BMT. These are located in:    
\noindent `/projects/rpci/lsuchest/lsuchest/Rserve/ImputeData/var/db/gwas/ \`  
`imputed_data/BMT093013_forImpute/Impute2_summary/Impute2.INFO`   

\noindent file. 

\singlespacing
```{bash head_impute2.info, eval=F}
...
\doublespacing
\noindent Now that we have the gene locations, we are going to create a shell script that will be able to grab the `Impute2.INFO` file regions and let us collect all of the SNPs that we have from DISCOVeRY-BMT that are typed and imputed. The shell script can be found at:  

\noindent `/projects/rpci/lsuchest/lsuchest/CandidateGeneReplication/ \`  
...
\singlespacing
```{r awk impute2.info by gene locations, eval=F}
...
\doublespacing
\noindent We edited this to make this into a SLURM script would and after running we have a file `info.cg.txt`. 
...
\doublespacing
\noindent `info.cg.txt` does not have a header so are going to add the header in R and re-write the file (alternatively, we could have just assigned this in each of the survival parsing R scripts that we are about to make.)  
...
\noindent Individual directory:     
\noindent `/projects/rpci/lsuchest/lsuchest/Rserve/ImputeData/var/db/gwas/ \`  
`imputed_data/BMT093013_forImpute/analyses/`  
\noindent Meta-individual directory:    
\noindent `/projects/rpci/lsuchest/lsuchest/Rserve/ImputeData/var/db/gwas/ \`  
`imputed_data/BMT093013_forImpute/analyses/METAL.results/`  
\noindent Shared directory:    
\noindent `/projects/rpci/lsuchest/lsuchest/Rserve/ImputeData/var/db/gwas/ \`  
`imputed_data/SHARED/analyses/METAL.RESULTS.SHARED/`  
\noindent Meta-shared directory:    
\noindent `/projects/rpci/lsuchest/lsuchest/Rserve/ImputeData/var/db/gwas/ \`    
`imputed_data/BMT093013_forImpute/analyses/METAL.results/`  

\noindent As a collective, these directories contain files of 144 analyses. However, the result files are not so well organized. Using some unix commands to capture a clean amount of directories and files, as well as *ad hoc* manual curation, final directory lists (`/projects/rpci/lsuchest/lsuchest/CandidateGeneReplication/ \`  
\noindent `survival_results_directories/`) can be found in the following files: `ind.directories.txt`, `shared.directories.txt`, `meta.ind.directories.txt`, and `meta.shared.directories.txt`.
...
\noindent The result files go into the directory: `/projects/rpci/lsuchest/abbasriz/\`
`candidate_gene/result_files_cg/impute.results.w.typedsnps`  

\noindent The result files will be in the following format:  
...
\doublespacing 
\noindent \textbf{Note}: Don't run the following shell script without editing the R script above (and similar ones) on UB CCR, as the directory may not be correct. The following shell script (`/projects/rpci/lsuchest/abbasriz/candidate_gene/ \`  
\noindent `result_files_cg/res.directories/independent_results.sh`) was written to run this command using SLURM:  
...
\noindent Again, the 144 result files can be found in: `/projects/rpci/lsuchest/abbasriz/\`  
`candidate_gene/result_files_cg/impute.results.w.typedsnps/`
...
\noindent `dups.sh`) and shared (`shared_remove_dups.R` and `shared_remove_dups.sh`) genomes as well. Again, the donor, recipient, and shared result files and their corresponding shell scripts are in `/projects/rpci/lsuchest/ \`
\noindent `abbasriz/candidate_gene/result_files_cg/impute.results.w.typedsnps`. The output for this analysis can be found in: `/projects/rpci/lsuchest/ \`
\noindent `abbasriz/candidate_gene/result_files_cg/`, however, the files in these directories are the most current files, and they have been updated from this point in the analysis and include meta results, hazard ratios, and 95% confidence intervals for the HRs. 
...
\noindent `result_files_cg/metal_results/` These analyses are run pairwise (cohort 1 and cohort 2), e.g. `/projects/rpci/lsuchest/ \`   
\noindent `abbasriz/candidate_gene/result_files_cg/D_c1_DRM_ALLonly.txt` and `/projects/rpci/ \`  
\noindent `lsuchest/abbasriz/candidate_gene/result_files_cg/D_c2_DRM_ALLonly.txt`.
...
\doublespacing
\noindent Another python script was written to create the shell script to run these METAL scripts
...
\doublespacing
\noindent The results are stored in `.tbl` files in `/projects/rpci/lsuchest/abbasriz/ \`   
...
\noindent `metal_results/merge_metal_res_w_impute_res.R`
...
\noindent `candidate_gene/result_files_cg/calc_hazardratios.R`. This R script also ensures both allele1 and allele2 are uppercase and reassigns the rows in the `impute` column to `typed` (repeated rsid) and `imputed` (---).
...
\noindent `candidate_gene/result_files_cg/descript_columns.R`
...
\noindent The shell script can be found at: `/projects/rpci/lsuchest/abbasriz/ \`  
\noindent `candidate_gene/result_files_cg/vegas2/v2_parse.sh` The results look like the following:  
...
\doublespacing
\noindent The vegas2 results were split into 3 shell scripts (`run_vegas2_donor.sh`, `run_vegas2_recipient.sh`, `run_vegas2_shared.sh`). To create these files we used perl.
...
\doublespacing
\noindent These were put into SLURM scripts and the following lines were run in the programs
...
\doublespacing
\noindent An example of the arguments that were used for vegas2 produced from the perl script can be seen here:  
...
\doublespacing
\noindent VEGAS2 results are stored in: `/projects/rpci/lsuchest/abbasriz/ \`  
\noindent `candidate_gene/result_files_cg/vegas2/results` with the extension `.V2out`.
...
\singlespacing
```{r echo=F, eval=F}
v2.res <- read.table("cg_v2table.txt", header=T)
head(v2.res)
```
...
```{r, echo=F, eval=F}
typed.snps <- read.table("typed.snps.txt", header=T)
head(typed.snps)
```

\doublespacing
\noindent We adjusted the topSNP pvalues by the number of typed SNPs in that gene (in cohort 1). We also adjusted the gene-based pvalue by the total number of genes that we tested in the candidate gene replication study. 
...
\singlespacing
```{r, echo=F, eval=F}
v2.typed.snps <- read.table("cg_v2table_adj.txt", header=T)
head(v2.typed.snps)
```
...
\doublespacing
\noindent Now that we have the vcf file, we can use `VariantAnnotation` library to easily pull this data into R.
\singlespacing
...
\doublespacing
\noindent Now we will grab the donor and recipients FID and IID. These correspond to the indices for the samples that we will subset.
...
\doublespacing
\noindent Now we we will grab the genotype threshold values
...
\doublespacing
\noindent We can see that we have both typed and imputed for rs2066844. We will keep only the imputed ones (which is the second rs2066844)
...
\doublespacing
\noindent Now we are going to transpose this data frame so we can have the samples as the rows and the SNPs as the columns
...
\doublespacing
\noindent Now we code the alleles in a dominant model. So if there is any 1 alleles, we will just relabel it as 1 and if there are homozygous 0, we will relabel as 0. We will also relabel the . as NA and then remove the NAs
...
\doublespacing
\noindent Now we will subset based off of the genome and cohort we are interested and create variables to do the composite SNP testing. So if SNP 8 and SNP 12 had 0 for both SNPs, they will be recoded as 0, and if any had 1 in it, it will be recoded as a 1. This will also done for the 3 SNPs (SNP8/SNP12/SNP13).
...
\doublespacing
\noindent Now we have the recoded SNP8 and SNP12. We need to append these back to the phenotype files so we can run survival analyses on these. Here we grab the recipient and donor phenotype files and merge them based off of pair_id column.
...
\doublespacing
\noindent Meta-analysis was performed using METAL software. The hazard ratios and 95\% confidence intervals were computed using the standard errors from the METAL output. 
...
\doublespacing
\noindent CCR5 meta analysis was run and the results are loaded in below:   
