...
```{r, echo=FALSE, fig.width=6, fig.height=8, fig.align="center", fig.cap="Disease Status Distribution for All Outcomes by Time to Event. The x-axis is time to death (DRM, TRM, OS) or relapse (PFS, REL)."}
library(tidyverse)
...
distPlot <- function(dat, outcome, variable, death_relapse,  legend_title){
    dat <- dat %>%
    mutate(cohort=ifelse(cohort==1, "Cohort \n 1", "Cohort \n 2"),
...
```{r, echo=FALSE, fig.width=6, fig.height=8, fig.align="center", fig.cap="Graft Source Distribution for All Outcomes by Time to Event. The x-axis is time to death (DRM, TRM, OS) or relapse (PFS, REL)."}

d1 <- distPlot(covs, "DRM", PBlood, "death",  "Graft Source")
d2 <- distPlot(covs, "TRM", PBlood, "death", "Graft Source")
d3 <- distPlot(covs, "OS", PBlood, "death", "Graft Source")
d4 <- distPlot(covs, "PFS", PBlood, "relapse", "Graft Source")
d5 <- distPlot(covs, "REL", PBlood, "relapse",  "Graft Source")
...
```{r, echo=FALSE, fig.width=6, fig.height=8, fig.align="center", fig.cap="Body Mass Index (BMI) Distribution for All Outcomes by Time to Event. The x-axis is time to death (DRM, TRM, OS) or relapse (PFS, REL)."}

d1 <- distPlot(covs, "DRM", bmi_cat, "death",  "Body Mass Index (BMI)")
d2 <- distPlot(covs, "TRM", bmi_cat, "death", "Body Mass Index (BMI)")
d3 <- distPlot(covs, "OS", bmi_cat, "death", "Body Mass Index (BMI)")
d4 <- distPlot(covs, "PFS", bmi_cat, "relapse", "Body Mass Index (BMI)")
d5 <- distPlot(covs, "REL", bmi_cat, "relapse",  "Body Mass Index (BMI)")
...
```{r, echo=FALSE, fig.width=6, fig.height=8, fig.align="center", fig.cap="Conditioning Intensity Distribution for All Outcomes by Time to Event. The x-axis is time to death (DRM, TRM, OS) or relapse (PFS, REL). MA is myeloablative, RIC is reduced intensity conditioning."}

d1 <- distPlot(covs, "DRM", CondInt, "death",  "Conditioning Intensity")
d2 <- distPlot(covs, "TRM", CondInt, "death", "Conditioning Intensity")
d3 <- distPlot(covs, "OS", CondInt, "death", "Conditioning Intensity")
d4 <- distPlot(covs, "PFS", CondInt, "relapse", "Conditioning Intensity")
d5 <- distPlot(covs, "REL", CondInt, "relapse",  "Conditioning Intensity")
...
c1 <- covariate.file %>%
    filter(sample_type=="recipient",
           cohort==1) %>%
    mutate(age_cat = ifelse(age >= 40, 1, 0),
           intxsurv_1Y_days=intxsurv_1Y*30.4167) # convert to daysc
