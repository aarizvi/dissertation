...
#qctool -g /projects/rpci/lsuchest/lsuchest/Rserve/ImputeData/var/db/gwas/imputed_data/BMT093013_forImpute/BMT093013_forImpute.chr3-45000000-50000000.impute2 -og /projects/rpci/lsuchest/abbasriz/candidate_gene/cg_haplotypes/ccr5_rep/ccr5_rep_dosages_threshold.vcf -incl-rsids /projects/rpci/lsuchest/abbasriz/candidate_gene/cg_haplotypes/ccr5_rep/ccr5_snps.txt --threshold 0.8
...
recs <- data.table(h1h1[ids$r_sample_index,], keep.rownames=T)
donors <- data.table(h1h1[ids$d_sample_index,], keep.rownames=T)
...
recs$Rh1h1 <- recs %>% select(grep("rs", colnames(recs))) %>% mutate(Rh1h1=ifelse(rowSums(.)==4, 1, 0)) %>% select(Rh1h1) %>% na.omit()  

donors$Dh1h1 <- donors %>% select(grep("rs", colnames(donors))) %>% mutate(Dh1h1=ifelse(rowSums(.)==4, 1, 0)) %>% select(Dh1h1) %>% na.omit() 

donors.h1h1 <- donors %>% select(pair_id, Dh1h1)
recs.h1h1 <- recs %>% select(pair_id, Rh1h1)

dr.h1h1 <- donors.h1h1 %>% inner_join(recs.h1h1, "pair_id") %>% data.table()
...
r.cov <- r.cov %>% inner_join(dr.h1h1, "pair_id") %>% data.table()
